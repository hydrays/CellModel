...
struct BoundaryData
{
    ...
    int set_boundary_data(double L_min_value, double L_max_value, double H_value)
	{
	    L_min = L_min_value;
	    L_max = L_max_value;
	    H = H_value;
...
public:
    ...
    int apply_mixed_boundary_condition_ellipse()
    {
	...
	{
	    if ( (ellipse_list[k].c2 < -H) || (ellipse_list[k].c2 > H) )
	    {
	...
	int Nsize = ellipse_list.size();
	for ( int k=0; k<Nsize; k++ )
	{
	    // left
	    if ( ellipse_list[k].type == 10 )
	    {
		// add the first extra layers
		Ellipse ellipse = ellipse_list[k];
		int id = ellipse_list[k].ellipse_id;
		double c1 = ellipse_list[k].c1;
		ellipse.c1 = c1 - 2.0;
		ellipse.ellipse_id = id + MAX_NUM_ELLIPSE;
		ellipse_list.push_back(ellipse);
		// add the second extra layers
		ellipse.c1 = c1 - 4.0;
		ellipse.ellipse_id = id + 10*MAX_NUM_ELLIPSE;
		ellipse_list.push_back(ellipse);
		//std::cout << "add boundary ellipse: " << ellipse.type << "id: " << id << "\n";
	    }
	    // right
	    if ( ellipse_list[k].type == 20 )
	    {
		// add the first extra layers
		Ellipse ellipse = ellipse_list[k];
		int id = ellipse_list[k].ellipse_id;
		double c1 = ellipse_list[k].c1;
		ellipse.c1 = c1 + 2.0;
		ellipse.ellipse_id = id + 2*MAX_NUM_ELLIPSE;
		ellipse_list.push_back(ellipse);
		// add the second extra layers
		ellipse.c1 = c1 + 4.0;
		ellipse.ellipse_id = id + 20*MAX_NUM_ELLIPSE;
		ellipse_list.push_back(ellipse);
		//std::cout << "add boundary ellipse: " << ellipse.type << "id: " << id << "\n";
	    }
	}
	Nsize = ellipse_list.size();
	for ( int k=0; k<Nsize; k++ )
	{
	    // top
    ...
    int apply_mixed_boundary_condition_voronoi()
    {
	double H = boundary_data.H;
	for ( int k=0; k<voronoi_cell_list.size(); k++ )
	{
	...
	{
	    id = id + 1;
	    cx = boundary_data.L_max + 2.0;
	    cy = -boundary_data.H + 2.0*(i+0.5);
	    double v1 = 1.0;
	    double v2 = 0.0;
	    double u1 = 0.0;
	    double u2 = 1.0;
