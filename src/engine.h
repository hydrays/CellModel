...
public:    
	...
	if ( params.flag_continue )
	{
	    /* if (!params.flag_record_final_state) */
	    /* { */
	    /* 	std::cout << "flag_record_final_state must set to 1 if flag_continue is set to 1"; */
	    /* 	exit(-1); */
	    /* } */
	    FILE *fp;
	    ...
	    if(fp == NULL)  
	    {
		geometry.boundary_data.L0 = geometry.boundary_data.L_max -
		    geometry.boundary_data.L_min + 2.0;
		params.t = 0;
		/* std::cout << "No file to continue with. Exit with ERROR!\n"; */
		/* exit(-1);  */
	    }
	    else
	    {
		double temp1, input_t, input_Lt;
		while(fscanf(fp, "%lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf",
			     &input_t, &temp1, &temp1, &temp1, &temp1, &temp1, &temp1, &temp1, &temp1,
			     &temp1, &temp1, &temp1,
			     &input_Lt) > 0);
		printf("record: %lf, %lf\n", input_t, input_Lt);
		params.t = input_t;
		geometry.boundary_data.L0 = input_Lt * exp(-params.mu*params.nu*params.t);
		fclose(fp);
	    }
	}
	else
	...
	for ( int istep=0; istep<params.Nstep; istep++ )
	{
	    ...
	    {
		geometry.output_cell_list(params);
		geometry.output_ellipse_list(params);
	    }
	    force.update_force_field(geometry, params);
	    if ( params.flag_output_cell )
	    {
		force.output_force_filed(geometry, params);
	    }
	    params.t = params.t + params.dt;
	    update_cell_position(geometry, force, params);

	    cell_division(geometry, force, params, div_info);
	    collect_info(geometry, force, params, sys_info);

	    foutput << params.t << ", " << div_info.counter << ", "
		    << div_info.div_index << ", "
		    ...
		    << geometry.boundary_data.L0*exp(params.mu*params.nu*params.t) << "\n";
	    foutput.flush();	    
	...
	if (params.flag_record_final_state)
	{
	    ...
	    //generate_continue_point();

	    std::ifstream infile(params.output_dir + "/statistics" + std::to_string(process_id) +".txt");
	    std::ofstream frecord;
	    frecord.open("record.txt", std::ofstream::out | std::ofstream::app);
	    if(!frecord || !infile)  
	    {
		std::cout << "record file open error.\n";
		return -1; 
	    }
	    std::string content = "";
	    ...
	    std::cout << "Program terminated by stage.\n";
	    exit(0);
	}
	return 0;
    }
    ...
    int update_cell_position(Geometry &geometry, Force &force, Parameters &params)
    {
	...
	for ( int k=0; k<geometry.voronoi_cell_list.size(); k++ )
	{
		...
		{
		    double Lexter = geometry.boundary_data.L0 * exp(params.mu*params.nu*params.t);
		    F = -params.alpha*(L_max_old - L_min_old - Lexter);
		    if ( F < 0.0 )
		    {
			F = 0.0;
		    }
		...
		if ( geometry.voronoi_cell_list[k].ellipse.type == 10 )
		{
		    ...
		    //5.0*params.eta*(geometry.voronoi_cell_list[k].ellipse.c1 - L_min_old);
		    //(F+2.0)*params.eta*(geometry.voronoi_cell_list[k].ellipse.c1 - L_min_old);
		...
		else if ( geometry.voronoi_cell_list[k].ellipse.type == 20 )
		{
		    ...
		    //5.0*params.eta*(geometry.voronoi_cell_list[k].ellipse.c1 - L_max_old);
		    //(F+2.0)*params.eta*(geometry.voronoi_cell_list[k].ellipse.c1 - L_max_old);
