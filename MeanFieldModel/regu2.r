...
for ( i in seq(Nstep) )
{
    ...
    if ( i%%10 == 0 )
    {
        ...
        ES <- mean((1-p0)*cos(theta2) - p0*sin(theta2))
        ##cat(A1, A2, S, ES, F, sig, thetabar, '\n')
        
        divtheta <- sample(theta2, 1)
...
## png("output2.png")
## plot(output[,1], type='o', ylim=c(-5, 5))
## lines(output[,2], col='green', lty=1)
## lines(output[,4], col='blue')
## dev.off()
